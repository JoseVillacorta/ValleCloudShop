...
class Clientes extends Controller
{
    ...
    public function registroDirecto()
    {
        ...
        if (isset($_POST['nombre']) && isset($_POST['clave'])) {
            if(empty($_POST['nombre']) || empty($_POST['correo']) || empty($_POST['clave'])){
                $mensaje = array('msg' => 'TODOS LOS CAMPOS SON REQUERIDOS', 'icono' => 'success');
            }else{
                $nombre = $_POST['nombre'];
                $correo = $_POST['correo'];
                $clave = $_POST['clave'];
                ...
                if(empty($verificar)){
                    ...
                    $data = $this->model->getregistroDirecto($nombre, $correo, $hash, $telefono, $dni, $edad, $token);
    ...
    public function enviarCorreo()
    {
        if (isset($_POST['correo']) && isset($_POST['token'])) {
            ...
            try {
                ...
                //Recipients
                $mail->setFrom('geoauron7@gmail.com', TITLE);
                ...
                $mail->send();
                $mensaje = array('msg' => 'CORREO ENVIAR REVISA TU BANDEJA DE ENTRADA - SPAM', 'icono' => 'success');
            } catch (Exception $e) {
        ...
        if (isset($_POST['correoLogin']) && isset($_POST['claveLogin'])) {
            if(empty($_POST['correoLogin']) || empty($_POST['claveLogin'])){
                ...
                if(!empty($verificar)){
                    ...
                    if (password_verify($clave, $verificar['contrasenia'])) {
    ...
    public function registrarPedido(){
        ...
        if (is_array($pedidos) && is_array($productos)) {
            ...
            $data = $this->model->registrarPedido($id_trasaccion, $estado, $monto, $fecha, $email,
                                            $nombre, $apellido, $direccion, $ciudad, $email_user);

            if($data > 0){
                foreach($productos as $producto){
                    $temp =$this->model->getProducto($producto['idProducto']);
                    $this->model->registrarDetalle($temp['namePro'], $temp['precio'], $producto['cantidad'], $data);
                }
